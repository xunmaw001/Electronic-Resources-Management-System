<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="提交审批时间" prop="tijiaoshenpishijian">
            <el-date-picker
                format="yyyy 年 MM 月 dd 日"
                value-format="yyyy-MM-dd"
                v-model="ruleForm.tijiaoshenpishijian" 
                type="date"
                :readonly="ro.tijiaoshenpishijian"
                placeholder="提交审批时间">
            </el-date-picker> 
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.tijiaoshenpishijian" label="提交审批时间" prop="tijiaoshenpishijian">
              <el-input v-model="ruleForm.tijiaoshenpishijian" 
                placeholder="提交审批时间" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      </el-row>
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.yonghuxingming = true;
          ...
          if(o=='tijiaoshenpishijian'){
            this.ruleForm.tijiaoshenpishijian = obj[o];
	    this.ro.tijiaoshenpishijian = true;
            continue;
          }
        }
      }
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `bianmujiancha/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.bianmujianchaCrossAddOrUpdateFlag = false;
						 this.parent.search();
